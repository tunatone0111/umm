...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_inference(
        ...
        # Check if we should extract attention weights for THIS timestep
        # This allows us to use fast Flash Attention for timesteps we're not saving
        extract_attention = getattr(self.config, "_should_extract_this_timestep", False)

        if extract_attention:
            # Manual attention computation to extract weights
            # Repeat key/value heads for GQA
            num_key_value_groups = self.num_heads // self.num_key_value_heads
            merged_key_states_expanded = merged_key_states[:, :, None, :].repeat(
                1, 1, num_key_value_groups, 1
            )
            merged_key_states_expanded = merged_key_states_expanded.reshape(
                -1, self.num_heads, self.head_dim
            )
            merged_value_states_expanded = merged_value_states[:, :, None, :].repeat(
                1, 1, num_key_value_groups, 1
            )
            merged_value_states_expanded = merged_value_states_expanded.reshape(
                -1, self.num_heads, self.head_dim
            )
            ...
            for i in range(len(query_lens)):
                ...
                q = packed_query_states[q_start:q_end]  # [seq_q, num_heads, head_dim]
                k = merged_key_states_expanded[
                    k_start:k_end
                ]  # [seq_k, num_heads, head_dim]
                v = merged_value_states_expanded[
                    k_start:k_end
                ]  # [seq_k, num_heads, head_dim]
                ...
                attn_scores = torch.matmul(
                    q.transpose(0, 1),  # [num_heads, seq_q, head_dim]
                    k.transpose(0, 1).transpose(-2, -1),  # [num_heads, head_dim, seq_k]
                ) / (self.head_dim**0.5)
                ...
                attn_output = torch.matmul(
                    attn_weights, v.transpose(0, 1)
                )  # [num_heads, seq_q, head_dim]
                attn_output = attn_output.transpose(
                    0, 1
                ).contiguous()  # [seq_q, num_heads, head_dim]
                attn_output_list.append(attn_output)

            packed_attn_output = torch.cat(attn_output_list, dim=0)
            packed_attn_output = packed_attn_output.reshape(-1, self.hidden_size)

            # Store attention weights in the model's storage
            if hasattr(self.config, "_attention_weights_storage"):
                if self.config._attention_weights_storage is None:
                    self.config._attention_weights_storage = {}
                self.config._attention_weights_storage[f"layer_{self.layer_idx}"] = (
                    attn_weights_list
                )
        else:
            # Use Flash Attention (original fast path)
            packed_attn_output = flash_attn_varlen_func(
                q=packed_query_states,
                k=merged_key_states,
                v=merged_value_states,
