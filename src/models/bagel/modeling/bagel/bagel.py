...
class Bagel(PreTrainedModel):
    ...
    def __init__(self, language_model, vit_model, config: BagelConfig):
        ...
        # Initialize attention weights storage (will be populated during inference if enabled)
    ...
    def generate_image(
        ...
        # Set up attention weights extraction
        # IMPORTANT: Set flags on language_model.config, not self.config!
        # The attention layers see language_model.config, not Bagel's config
        if extract_attention_weights:
            self.language_model.config.extract_attention_weights = True
            self.language_model.config._attention_weights_storage = {}
            self.language_model.config._should_extract_this_timestep = (
                False  # Will be set per-timestep
            )
            self.attention_weights_storage = {}
            # Set which timesteps to extract (None means all timesteps)
            # NOTE: For num_timesteps=N, valid indices are 0 to N-2 (because timesteps[:-1] removes last)
            if extract_timesteps is not None:
                max_valid_idx = (
                    num_timesteps - 2
                )  # Because timesteps[:-1] removes last timestep
                invalid_indices = [
                    idx for idx in extract_timesteps if idx > max_valid_idx
                ]
                if invalid_indices:
                    import warnings

                    warnings.warn(
                        f"Warning: Some timestep indices are invalid for num_timesteps={num_timesteps}. "
                        f"Valid range: [0, {max_valid_idx}]. Invalid indices: {invalid_indices}. "
                        f"These will be ignored."
                    )
                # Filter out invalid indices
                self.extract_timesteps = set(
                    [idx for idx in extract_timesteps if 0 <= idx <= max_valid_idx]
                )
            else:
                self.extract_timesteps = None
        else:
            self.language_model.config.extract_attention_weights = False
            self.language_model.config._should_extract_this_timestep = False
            self.extract_timesteps = None
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            # Determine if we should extract attention for THIS specific timestep
            should_extract_this_timestep = extract_attention_weights and (
                self.extract_timesteps is None or i in self.extract_timesteps
            )

            # Set flag for attention layers to decide between Flash Attention and manual attention
            # Set on language_model.config so attention layers can see it
            if extract_attention_weights:
                self.language_model.config._should_extract_this_timestep = (
                    should_extract_this_timestep
                )
            ...
            # Collect attention weights for this timestep (if we extracted it)
            if should_extract_this_timestep and hasattr(
                self.language_model.config, "_attention_weights_storage"
            ):
                for (
                    layer_key,
                    layer_attn_weights,
                ) in self.language_model.config._attention_weights_storage.items():
                    timestep_key = f"{layer_key}_timestep_{i}"
                    self.attention_weights_storage[timestep_key] = layer_attn_weights

                # Reset storage for next timestep
                self.language_model.config._attention_weights_storage = {}
